...
public class ViewAddQuesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_add_ques);
...
// liệu vào cơ sở dữ liệu. Sau khi lưu thành công, giao diện được reset lại về trạng thái ban đầu.
    private void saveQuestionToDatabase() {
        ...
        if(question.isEmpty() || answer.isEmpty() || hint.isEmpty() || imgName == null) {
            Toast.makeText(ViewAddQuesActivity.this, "Vui lòng nhập dầy đủ thông tin và chọn ảnh", Toast.LENGTH_SHORT).show();
            return;
        }

        DatabaseHelper dpHelper = new DatabaseHelper(ViewAddQuesActivity.this);
        dpHelper.getWritableDatabase();
        boolean success = dpHelper.addGame(imgName, question, answer, hint);
        if(success) {
            Toast.makeText(ViewAddQuesActivity.this, "Lưu câu hỏi thành công!", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(ViewAddQuesActivity.this, "Lưu câu hỏi thất bại", Toast.LENGTH_SHORT).show();
        }
    }
}